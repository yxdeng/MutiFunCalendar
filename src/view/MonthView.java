...
public class MonthView extends View {
    ...
    /**
     * use to init the MonthView
     * the CalendarActivity call at onCreate
    ...
    /**
     * se to init the MonthView
     * the CalendarActivity call at update
     * @param context the Activity
     * @param cal the NomalCalendar
     */
    public MonthView(Context context, NomalCalendar cal) {
        super(context);
        myCal = cal;
        init(context);
        isSelected = true;
        this.weeks = myCal.getWeeks();
    }

    /**
     * use to init the command parameters
     * isfocuseable , the tochmode, the headpaint ,background
     * @param context
     */
    private void init(Context context) {
        this.setId(83);
        setBackgroundResource(org.me.mutifuncalendar.R.drawable.background);
        this.main = (CalendarActivity) context;
        setFocusable(true);
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        if (bundle.getInt("selectday") == bundle.getInt("currentday")) {
            MonthObservable mob = new MonthObservable(this.myCal);
            mob.addObserver(main);
            mob.change();
        }
        isSelected = true;
    ...
    private void drawMonth(Canvas canvas, float oldheight, float rheight) {
        ...
        canvas.drawText(String.valueOf(myCal.getCurrent_month() + 1),
                getWidth() / 2f,
                (rheight / 2f) + 2 * oldheight,
                montPaint);
    }
    ...
    private void drawSelectRect(Canvas canvas) {
        if (isSelected) {
            setSelectDay(myCal.getCurrent_day());
            isSelected = false;
        }
